...
export default function AdminPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (devAuth) {
        ...
        } catch { localStorage.removeItem('wmm_dev_auth'); }
      }
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session?.user) {
        ...
        if (!localStorage.getItem('wmm_dev_auth')) navigate('/login');
      }
    });
  ...
  const handleAdd = () => {
    setEditProject(null); setEditNews(null); setEditCareer(null); setEditLegacy(null);
    setView('add');
  };
  ...
  const handleDeleteProject = async () => {
    ...
    setDeleteProjectId(null); setDeletingProject(false);
    fetchProjects(); showToast('Proyek berhasil dihapus.');
  };
  ...
  const handleDeleteNews = async () => {
    ...
    setDeleteNewsId(null); setDeletingNews(false);
    fetchNews(); showToast('Artikel berhasil dihapus.');
  };
  ...
  const handleDeleteCareer = async () => {
    ...
    setDeleteCareerID(null); setDeletingCareer(false);
    fetchCareers(); showToast('Lowongan berhasil dihapus.');
  };
  ...
  const handleDeleteLegacy = async () => {
    ...
    setDeleteLegacyId(null); setDeletingLegacy(false);
    fetchLegacy(); showToast('Proyek sejarah berhasil dihapus.');
  };
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (isResizingSidebar) {
        ...
        const newWidth = Math.max(280, Math.min(640, resizeStartWidthRef.current + delta));
  ...
  const pushThemeToPreview = async () => {
  ...
  return (
    <div className="h-screen w-screen bg-[#080C14] text-white flex flex-col overflow-hidden">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-[60] bg-amber-400 text-black text-xs font-bold px-4 py-2.5 rounded-xl flex items-center gap-2 shadow-lg">
