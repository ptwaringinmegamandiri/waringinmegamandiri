    ...
    <footer className="relative bg-[#050A14] border-t border-white/5 pt-14 pb-8">
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 mb-12">
          ...
          {/* Kolom Kontak - WARNA DISERAGAMKAN */}
          <div className="lg:col-span-3">
            ...
            <div className="space-y-3 text-sm text-slate-400 font-body">
              <div className="flex gap-2">
                <i className="ri-map-pin-line text-sky-400" />
                <p>{t('kontak.addressVal')}</p>
              </div>
              <div className="flex gap-2">
                <i className="ri-phone-line text-sky-400" />
                <p>{t('kontak.phoneVal')}</p>
              </div>
              <div className="flex gap-2">
                <i className="ri-mail-line text-sky-400" />
                <p>{t('kontak.emailVal')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-slate-800/80 pt-6 flex flex-col sm:flex-row justify-between items-center text-slate-600 text-xs gap-3">
          ...
          <div className="flex gap-6">
            <span className="hover:text-sky-400 cursor-pointer transition-colors">{t('footer.privacy')}</span>
            <span className="hover:text-sky-400 cursor-pointer transition-colors">{t('footer.terms')}</span>
          </div>
        </div>
      </div>
    </footer>
